...
namespace CSLShowCommuterDestination.Game
{
    ...
    {
        private static Vector3 HEIGHT_OFFSET = new Vector3(0, 150f, 0);
                ...
                for (int index2 = num2; index2 <= num4; ++index2)
                {
                    ...
                    while (citizenInstanceId != (ushort)0)
                    {
                        ...
                        )
                        {
                            ...
                            {
                                var stop = new DestinationStop(
                                    destinationStopId,
                                    Bridge.GetStopPosition(destinationStopId) + HEIGHT_OFFSET,
                                    Bridge.GetStopLineColor(destinationStopId)
                                );

                                stops.Add(destinationStopId, stop);
                            }

                            var targetBuildingId = citizen.m_targetBuilding;

                            stops[destinationStopId].AddJourney(targetBuildingId, Bridge.GetBuildingPosition(targetBuildingId) + HEIGHT_OFFSET);
                        }
